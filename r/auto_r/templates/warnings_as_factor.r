...
sink("{summary_path}/warnings_models_as_factor_all_repo.txt")
summary(avgDevDelta)

# Add the new warning counts
print("Predicting warnings")
mb = glm(warnings ~ ns + nd + nf + entrophy + la + ld + lt + fix + ndev + age + nuc + exp + rexp + sexp + contains_bug
...
mb = glm(new_warnings ~ ns + nd + nf + entrophy + la + ld + lt + fix + ndev + age + nuc + exp + rexp + sexp
...
mb = glm(security_warnings ~ ns + nd + nf + entrophy + la + ld + lt + fix + ndev + age + nuc + exp + rexp + sexp
+ contains_bug + as.factor(repo_name), family=binomial(),
control = list(maxit = 50))
summary(mb)
#vif(mb)
...
mb = glm(new_security_warnings ~ ns + nd + nf + entrophy + la + ld + lt + fix + ndev + age + nuc + exp + rexp + sexp
+ contains_bug + as.factor(repo_name), family=binomial(),
control = list(maxit = 50))
summary(mb)
#vif(mb)
