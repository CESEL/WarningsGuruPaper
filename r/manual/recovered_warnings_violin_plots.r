...
b_commons_lang = result[['commons-lang']]$warnings
b_hadoop = result[['hadoop']]$warnings
b_ignite = result[['ignite']]$warnings
b_kylin = result[['kylin']]$warnings
b_phoenix = result[['phoenix']]$warnings
b_ranger = result[['ranger']]$warnings
b_tika = result[['tika']]$warnings
b_wicket = result[['wicket']]$warnings
...
n_commons_lang = result[['commons-lang']]$new_warnings
n_hadoop = result[['hadoop']]$new_warnings
n_ignite = result[['ignite']]$new_warnings
n_kylin = result[['kylin']]$new_warnings
n_phoenix = result[['phoenix']]$new_warnings
n_ranger = result[['ranger']]$new_warnings
n_tika = result[['tika']]$new_warnings
n_wicket = result[['wicket']]$new_warnings
...
pdf('/home/louisq/git/louis/authorship/thesis/r/figures/recovered_new_warnings_violin.pdf', width=11)
...
title("Total Warnings and New Warnings in Commits", xlab="Projects", ylab="Number of warnings + 1 (log)")
legend("topright", fill = c("darkorange3", "lightblue"),
       legend = c("Total Warnings", "New Warnings"), box.lty=0)
dev.off()
